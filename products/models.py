...
# with the fixtures file in place and the products app created we need to create some models for the fixtures to go in 
...
class Category(models.Model):
    ...
    # below I'm also going to make a quick model method here which is the same thing as the string method
...
# See Below The product model isn't much more complex other than just having a few more fields.
...
# As usual, the string method will just return the products name so I'll copy that from the category model.
# And it's worth noting what we're requiring here
# each product requires a name, a description, and a price.
# But everything else is optional. eg see null=true and blank = true below 
